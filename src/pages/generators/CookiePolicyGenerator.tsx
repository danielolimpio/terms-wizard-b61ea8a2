...
export default function CookiePolicyGenerator() {
  ...
  if (generatedPolicy) {
    return (<div className="min-h-screen bg-background"><Header /><main className="container mx-auto px-4 py-8"><GeneratedPolicyResult generatedPolicy={generatedPolicy} onGenerateNew={() => setGeneratedPolicy(null)} /></main><Footer /></div>);
  }

  return (<div className="min-h-screen bg-background"><Header /><main className="container mx-auto px-4 py-8"><div className="mb-6"><Button variant="outline" onClick={() => window.history.back()}>← Voltar</Button></div><PolicyGenerator policyType={policyType} onGenerate={(formData) => setGeneratedPolicy({ id: Date.now().toString(), type: policyType, content: generateCookiePolicy(formData), formData, createdAt: new Date() })} /></main><Footer /></div>);
}